"""A tensor buffer that acts like a streaming batch."""
...
class StreamingBatch:
    """Streaming batch for PumaLoss: dict of tensors (capacity, seq_len) with eviction state.

    Can be created without capacity/seq_len/device; these are set on first evict_and_fill
    from the batch shape and used to initialize storage.
    """
    ...
    def __init__(
        ...
        # FlexAttention BlockMask for the current buffer, built in the loss from
        # ``storage['segment_ids']`` after ``evict_and_fill`` (row-aligned; safe after partial
        # eviction). Not stored as tensor rows; None when not using flex.
        self.block_mask: Optional[Any] = None
