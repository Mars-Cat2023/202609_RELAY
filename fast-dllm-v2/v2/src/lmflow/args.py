...
class FinetunerArguments(TrainingArguments):
    ...
    bptt_inner_block_size: int = field(
        default=8,
        metadata={
            "help": (
                "Inner block size used by bptt_unmask_strategy='decode_aligned'. "
                "Ignored by the default 'bd' strategy."
            )
        },
    )
    ...
    evalplus_on_save: bool = field(
        default=False,
        metadata={"help": "Submit an EvalPlus Slurm job after each saved checkpoint."},
    )
    evalplus_datasets: str = field(
        default="humaneval mbpp",
        metadata={"help": "Whitespace- or comma-separated EvalPlus datasets for on-save eval."},
    )
    evalplus_seeds: str = field(
        default="0",
        metadata={"help": "Whitespace- or comma-separated random seeds for on-save EvalPlus."},
    )
    evalplus_threshold: float = field(
        default=0.85,
        metadata={"help": "Confidence threshold passed to EvalPlus generation."},
    )
    evalplus_sbatch: str = field(
        default="train_scripts/evalplus_checkpoint.sbatch",
        metadata={"help": "Slurm wrapper used for on-save EvalPlus jobs."},
    )
    evalplus_block_cache: str = field(
        default="auto",
        metadata={"help": "EvalPlus block-cache mode: auto, on, or off."},
    )
    evalplus_use_carry: bool = field(
        default=False,
        metadata={"help": "Whether on-save EvalPlus should enable carry inference."},
    )
    evalplus_strict: bool = field(
        default=False,
        metadata={"help": "Fail training if on-save EvalPlus submission fails."},
    )
