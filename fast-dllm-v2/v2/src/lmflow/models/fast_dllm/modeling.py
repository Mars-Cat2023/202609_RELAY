...
class CausalLMOutputWithPastAndBlockCache(CausalLMOutputWithPast):
    ...
    carry_delta_raw_norm: Optional[torch.FloatTensor] = None
    carry_delta_mask_norm: Optional[torch.FloatTensor] = None
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # Keep activations in q/k's dtype: mixed-precision mul with cos/sin can promote to float32, which then
    # disagrees with value_states in bf16 and trips flex_attention / SDPA dtype checks in eager.
        ...
        **kwargs
        ...
        if h_t is not None and (self.use_cab or self.use_loopholing or self.use_mlp_carry):
            ...
            with torch.no_grad():
                ...
                guarded_delta = torch.where(guard, delta, torch.zeros_like(delta))
                carry_delta_mask_norm = guarded_delta.detach().float().norm() / denom
        ...
        return BaseModelOutputWithPastAndBlockCache(
            ...
            h_s=h_s,
            carry_delta_raw_norm=carry_delta_raw_norm,
            carry_delta_mask_norm=carry_delta_mask_norm,
        )
...
class Fast_dLLM_QwenForCausalLM(Fast_dLLM_QwenPreTrainedModel, GenerationMixin):
    ...
    @staticmethod
    def _tokenizer_from_config(config) -> Optional[PreTrainedTokenizerBase]:
        """Load ``AutoTokenizer`` from ``config._name_or_path`` when set (e.g. after ``from_pretrained``)."""
        ...
        try:
            ...
            logger.debug(
                "Fast_dLLM_QwenForCausalLM: could not load tokenizer from %r: %s",
                name,
                e,
            )
            return None
    ...
    def __init__(self, config):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        # Same hub/local id as the model weights (when from_pretrained was used). Assign manually
        # if you built the model from a bare config. Used by :meth:`decode_ids_for_debug`.
        self.tokenizer: Optional[PreTrainedTokenizerBase] = self._tokenizer_from_config(config)
    ...
    def decode_ids_for_debug(
        ...
        *,
        ...
        """Map one batch row of token ids to text (same idea as :class:`FastDLLMBlockBPTTLoss`)."""
        if self.tokenizer is None:
            return (
                "<no tokenizer: assign model.tokenizer=... or load via from_pretrained with a name/path>"
            )
    ...
    def forward(
        ...
        h_t: Optional[torch.Tensor] = None,
        bypass_noising: bool = False,
        **kwargs
    ) -> CausalLMOutputWithPastAndBlockCache:

        if self.training and not bypass_noising:
        ...
        if self.training and labels is not None:
            # Halve only when the doubled [x_t || x0] layout was actually used.
            # On the BPTT path we always pass labels of shape (B, L) alongside
            # input_ids of shape (B, 2L), so this still fires correctly.
            hidden_states = hidden_states[:, :hidden_states.shape[1]//2, :]
        # Only compute necessary logits, and do not upcast them to float if we are not computing the loss
        slice_indices = slice(-logits_to_keep, None) if isinstance(logits_to_keep, int) else logits_to_keep
        logits = self.lm_head(hidden_states[:, slice_indices, :])

        # Halve h_s to match the x_t half of the doubled sequence.
        h_s = outputs.h_s
        if h_s is not None and self.training and labels is not None:
            h_s = h_s[:, :h_s.shape[1]//2, :]
        ...
        return CausalLMOutputWithPastAndBlockCache(
            ...
            h_s=h_s,
            carry_delta_raw_norm=outputs.carry_delta_raw_norm,
            carry_delta_mask_norm=outputs.carry_delta_mask_norm,
        )
