...
class Fast_dLLM_QwenConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        bd_size=32,
        # Cross Attention Bridge (CAB) for BPTT hidden-state carry
        use_cab=False,
        ...
        # Position-guarded LayerNorm loopholing (Ablation A)
        use_loopholing=False,
        loophole_layer=-1,
        loophole_position_guard="mask",
        # MLP-bottleneck carry: h_t through CAB-style MLP, LayerNorm, add (like loopguard with bottleneck)
        use_mlp_carry=False,
        **kwargs,
    ):
        ...
        self.loophole_layer = loophole_layer
        if loophole_position_guard not in ("mask", "mutable"):
            raise ValueError(
                "loophole_position_guard must be 'mask' or 'mutable', "
                f"got {loophole_position_guard!r}"
            )
        self.loophole_position_guard = loophole_position_guard
        self.use_mlp_carry = use_mlp_carry
