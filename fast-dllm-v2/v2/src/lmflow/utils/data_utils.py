...
def preview_file(file_path: str, chars: int = 100):
    """
    Returns the first and last ``chars`` *bytes* of a file decoded as UTF-8
    with ``errors='replace'``, without loading the entire file into memory.

    Note on the previous implementation: it opened the file in text mode
    (``encoding='utf-8'``) and then did a *byte-offset* ``f.seek(file_size
    - chars)``. If that offset landed inside a multi-byte UTF-8 codepoint
    (common for non-ASCII content -- e.g. multilingual / math-symbol /
    code data such as the Tulu-3-SFT-Mixture shards), the subsequent
    ``f.read(chars)`` raised ``UnicodeDecodeError: 'utf-8' codec can't
    decode byte 0xXX in position 0: invalid start byte``. ASCII-only
    tails (e.g. the ``]}\n`` region of the Magicoder fixture) happened
    to dodge this.

    The two consumers (``get_dataset_type_fast`` /
    ``check_dataset_instances_key_fast``) only regex-search the returned
    strings for ASCII patterns (``"type":``, ``"instances"``), so a
    lossy ``errors='replace'`` decode is exactly what we want.

    Args:
        file_path (str): Path to the file to be previewed.
        chars (int, optional): Number of *bytes* to show from start and
            end. Defaults to 100. (Kept the name ``chars`` so existing
            call sites don't have to change.)

    Returns:
        tuple: (first_chars, last_chars) - the first and last bytes of
            the file decoded as UTF-8 with ``errors='replace'``.
    """
    file_size = os.path.getsize(file_path)

    with open(file_path, 'rb') as f:
        first_bytes = f.read(chars)

        if file_size <= 2 * chars:
            return first_bytes.decode('utf-8', errors='replace'), ""

        last_chunk_position = max(0, file_size - chars)
        f.seek(last_chunk_position)
        last_bytes = f.read(chars)

    return (
        first_bytes.decode('utf-8', errors='replace'),
        last_bytes.decode('utf-8', errors='replace'),
    )
