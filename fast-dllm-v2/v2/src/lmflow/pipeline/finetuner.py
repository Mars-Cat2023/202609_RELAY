...
# Importing this package registers the vendored Fast_dLLM_QwenConfig /
# Fast_dLLM_QwenForCausalLM classes with HF Auto* before any
# `from_pretrained(..., trust_remote_code=False)` call resolves them.
import lmflow.models.fast_dllm  # noqa: F401
...
class Finetuner(BaseTuner):
    ...
    def tune(self,
        ...
        if getattr(training_args, "loss_type", "mlm") == "bptt":
            # Forward 1 leaves the carry module unused, so DDP needs to
            # be told not to error on params with no grad in the first pass.
            training_args.ddp_find_unused_parameters = True

            backend_model = model.get_backend_model()
            if hasattr(backend_model, "gradient_checkpointing_enable"):
                backend_model.gradient_checkpointing_enable(
            ...
            def FinetuningTrainer(**kw):
                return FastDLLMBPTTTrainer(
                    mask_token_id=data_args.mask_id,
        ...
        if getattr(training_args, "evalplus_on_save", False):
            ...
            trainer_callbacks.append(
                EvalPlusOnSaveCallback(
                    run_name=training_args.run_name or "train",
                    use_carry=evalplus_use_carry,
                    datasets=training_args.evalplus_datasets,
                    seeds=training_args.evalplus_seeds,
                    threshold=training_args.evalplus_threshold,
                    sbatch_path=training_args.evalplus_sbatch,
                    block_cache=training_args.evalplus_block_cache,
                    strict=training_args.evalplus_strict,
                )
            )
