#!/usr/bin/env bash
# Submit the four 2-GPU c40m60 ablations with lenient gradient clipping.
#
# This family keeps the original BD-block unmask selection and changes only
...
recipe_tag=${RECIPE_TAG:-opencode_openmath_60k_c40m60_gradclip5}
...
common_export="ALL,DATASET_PATH=${dataset_path},RECIPE_TAG=${recipe_tag},BLOCK_SIZE=${BLOCK_SIZE:-2048},DISABLE_GROUP_TEXTS=${DISABLE_GROUP_TEXTS:-1},NUM_TRAIN_EPOCHS=${NUM_TRAIN_EPOCHS:-3},LEARNING_RATE=${LEARNING_RATE:-5e-6},MAX_GRAD_NORM=${MAX_GRAD_NORM:-5.0},SAVE_STEPS=${SAVE_STEPS:-200},SAVE_TOTAL_LIMIT=${SAVE_TOTAL_LIMIT:-0},PER_DEVICE_TRAIN_BATCH_SIZE=${PER_DEVICE_TRAIN_BATCH_SIZE:-2},GRADIENT_ACCUMULATION_STEPS=${GRADIENT_ACCUMULATION_STEPS:-16},BPTT_UNMASK_STRATEGY=bd"
...
echo
echo "Eval is NOT auto-submitted. After checkpoints are written, launch evals manually."
