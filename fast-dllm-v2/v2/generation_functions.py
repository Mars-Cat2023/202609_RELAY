...
class Fast_dLLM_QwenForCausalLM:
    ...
    def batch_sample(
        ...
        finished_samples = {}
        nfe_counts = torch.zeros((batch_size,), device=self.device, dtype=torch.long)
        for block_idx in range(start_block_idx, num_blocks):
                ...
                for small_block_idx in range(num_small_blocks):
                    ...
                    while True:
                        ...
                        if mask_idx[:, start:end].sum() == 0:
                            break
                        active_nfe_idx = sample_indices[
                            mask_idx[:, start:end].any(dim=1) & ~finished_flag
                        ]
                        ...
                        if use_block_cache:
                            ...
                            # If we have a carry, right-shift it (one position)
                            # before feeding it as ``h_t`` so the carried state
                            # at position ``i`` corresponds to "the rep used to
                            # predict token i", aligning with the right-shifted
                            # logits computed below. Same convention as the
                            # training-time BPTT loss / eval.py.
                            h_t_in = None
                            if _has_carry and h_s_carry is not None:
                                h_t_in = torch.cat(
                                    [h_s_carry[:, :1, :], h_s_carry[:, :-1, :]],
                                    dim=1,
                                )
                            _fwd_out = self.forward(
                            ...
                            logits = _fwd_out.logits
                            # Stale remote-code snapshots may omit h_s on the output object;
                            # use getattr and prefer --code_model_path / repo hf_baseline.
                            h_s_out = getattr(_fwd_out, "h_s", None)
                            if _has_carry and h_s_out is not None:
                                h_s_carry = h_s_out
                            logits = torch.cat([logits[:, :1, :], logits[:, :-1, :]], dim=1)
                            logits = logits[:, start:end]
            ...
            if finished_flag.any():
                ...
                if h_s_carry is not None:
                    h_s_carry = h_s_carry[~finished_flag]
        ...
        if return_nfe_stats:
            return finished_samples, {"nfe": nfe_counts.detach().cpu().tolist()}
